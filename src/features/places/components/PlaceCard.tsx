...
type PlaceCardNavigation = NativeStackNavigationProp<RootStackParamList, 'Discover'>

export default function PlaceCard({ cardData, index }: PlaceCardProps) {
  const image = cardData.photo?.images?.medium?.url
    ? cardData.photo.images.medium.url
    : 'https://cdn.pixabay.com/photo/2015/10/30/12/22/eat-1014025_1280.jpg'
  const title = cardData.name ?? ''
  const location = cardData.location_string ?? ''

  const navigation = useNavigation<PlaceCardNavigation>()
